...
// TEMPORARY DEBUG — traces the live Veltema/Lee co-tenant match against
// today's real attachments and emails the trace to ADMIN_EMAIL. Delete
// once the co-leaseholder bug is confirmed fixed.
function debugCheckVeltemaLee() {
  var sheet2Map = loadSheet2_();
  if (!sheet2Map) { Logger.log('Could not load Sheet2.'); return; }

  var attachments = findEmailAttachmentsBySubject_(ALL_LABELS, true);
  var directory   = attachments.directoryText   ? parseTenantDirectory_(attachments.directoryText) : [];
  var delinquents = attachments.delinquencyText ? parseDelinquencyCSV_(attachments.delinquencyText, sheet2Map, false, 0)
                                                 : { resolved: [], flagged: [] };

  var lines = [];
  var veltemaRow = delinquents.resolved.filter(function(r) {
    return normalizeForMatch_(r.primaryName).indexOf('veltema') >= 0;
  })[0];

  if (!veltemaRow) {
    lines.push('Veltema NOT in resolved delinquency rows (threshold=0).');
    var flagged = delinquents.flagged.filter(function(f) {
      return normalizeForMatch_(f.name).indexOf('veltema') >= 0;
    })[0];
    lines.push(flagged ? ('Flagged instead: ' + JSON.stringify(flagged)) : 'Not found flagged either — check attachments fetched OK.');
  } else {
    lines.push('Veltema resolved row: ' + JSON.stringify(veltemaRow));

    var relevant = directory.filter(function(e) {
      return normalizeForMatch_(e.tenant).indexOf('veltema') >= 0 || normalizeForMatch_(e.tenant).indexOf('lee') >= 0;
    });
    lines.push('Directory entries matching "veltema" or "lee": ' + JSON.stringify(relevant));

    var cache = {};
    relevant.forEach(function(e) {
      var key = resolveDirectoryPropertyKey_(e.property, sheet2Map, cache);
      lines.push('property="' + e.property + '" unit="' + e.unit + '" tenant="' + e.tenant +
                 '" -> resolvedKey=' + key + '  (Veltema sheetKey=' + veltemaRow.sheetKey + ', unit=' + veltemaRow.unit + ')');
    });

    var allTenants = lookupAllTenants_(veltemaRow.sheetKey, veltemaRow.unit, directory, sheet2Map, cache);
    lines.push('lookupAllTenants_ result: ' + JSON.stringify(allTenants));
  }

  var report = lines.join('\n\n');
  Logger.log(report);
  var admin = cfg_('ADMIN_EMAIL');
  if (admin) GmailApp.sendEmail(admin, 'DEBUG — Veltema/Lee co-tenant match trace', report);
}
